...
class NotificationSettings {
    constructor() {
        ...
        this.toggleNotificationsBtn = document.getElementById('toggleNotificationsBtn');
        ...
        this.locationFilterEnabled = document.getElementById('locationFilterEnabled');
        this.locationFilterOptions = document.getElementById('locationFilterOptions');
    ...
    async checkServiceWorkerRegistration() {
        ...
        if (!('serviceWorker' in navigator)) {
            this.updateRegistrationStatus('not-supported', 'Service Worker not supported');
            return;
        }
    ...
    async loadSettings() {
        try {
            ...
            this.updateToggleButton(this.settings.desktopNotifications || this.settings.pushNotifications);
    ...
    validateSettings(settings) {
        ...
        if (!settings.keywordFilter || typeof settings.keywordFilter.enabled !== 'boolean') {
    ...
    updateRegistrationStatus(status, customMessage = null) {
        ...
        const statusMap = {
            'registered': { text: 'Registered', icon: 'fa-check-circle', color: 'text-green-600' },
            'not-registered': { text: 'Not registered', icon: 'fa-times-circle', color: 'text-red-600' },
            'not-supported': { text: customMessage || 'Not supported', icon: 'fa-times-circle', color: 'text-red-600' },
            'error': { text: customMessage || 'Error', icon: 'fa-exclamation-circle', color: 'text-red-600' }
        };
    ...
    initializeEventListeners() {
        ...
        if (this.localeMenuButton && this.localeMenuDropdown) {
            ...
            document.addEventListener('click', (e) => {
                if (!this.localeMenuButton.contains(e.target) && !this.localeMenuDropdown.contains(e.target)) {
                    close();
                }
            });
            this.localeMenuDropdown.querySelectorAll('a[data-locale]').forEach(a => {
        ...
        if (this.testNotificationBtn) {
            this.testNotificationBtn.addEventListener('click', async () => {
                const isEnabled = this.settings.desktopNotifications || this.settings.pushNotifications;
                if (!isEnabled) {
                    console.log('Cannot show test notification - notifications are disabled');
                    return;
                }
        ...
        if (selectAllBtn) {
            selectAllBtn.addEventListener('click', () => {
                const checkboxes = this.locationFilterOptions.querySelectorAll('[name="locations"]');
        ...
        if (this.keywordSearch) {
            this.keywordSearch.addEventListener('input', (event) => {
                ...
                const items = this.keywordsContainer.querySelectorAll('[name="keywords"]');
                items.forEach(item => {
                    const label = item.nextElementSibling;
    ...
    updateUI() {
        ...
        if (this.testNotificationBtn) {
            ...
            if (isEnabled) {
                ...
                this.testNotificationBtn.className = 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors';
                icon.className = 'fas fa-paper-plane mr-1';
                this.testNotificationBtn.textContent = 'Send Notification';
                this.testNotificationBtn.prepend(icon);
            }
    ...
    async checkPushStatus() {
        try {
            ...
            if (data && data.push_enabled === false) {
                ...
                this.showError('Push notifications are inactive. Please enable them again.');
            }
        } catch (err) {
    ...
    updatePermissionStatus(status, customMessage = null) {
        ...
        const statusMap = {
            'granted': { text: 'Granted', icon: 'fa-check-circle', color: 'text-green-600' },
            'denied': { text: 'Denied', icon: 'fa-times-circle', color: 'text-red-600' },
            'default': { text: 'Not requested', icon: 'fa-question-circle', color: 'text-yellow-600' },
            'not-supported': { text: customMessage || 'Not supported', icon: 'fa-times-circle', color: 'text-red-600' }
        };
    ...
    updateToggleButton(isEnabled) {
        ...
        if (isEnabled) {
            ...
            this.toggleNotificationsBtn.className = 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors';
    ...
    updateLocationFilterButton(isEnabled) {
        ...
        if (isEnabled) {
            ...
            this.locationFilterEnabled.className = 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors';
            icon.className = 'fas fa-times mr-1';
    ...
    updateKeywordFilterButton(isEnabled) {
        ...
        if (isEnabled) {
            this.keywordFilterEnabled.className = 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 hover:bg-blue-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors';
            icon.className = 'fas fa-check mr-1';
            this.keywordFilterEnabled.textContent = 'Enabled';
            this.keywordFilterEnabled.prepend(icon);
            this.keywordFilterOptions.classList.remove('hidden');
        } else {
            this.keywordFilterEnabled.className = 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors';
            icon.className = 'fas fa-times mr-1';
    ...
    updateKeywordsUI() {
        ...
        const all = Array.from(new Set([...this.availableKeywords, ...this.settings.keywords])).sort();
    ...
    updatePermissionButton(permission) {
        ...
        switch (permission) {
            case 'granted':
                this.requestPermissionBtn.classList.add('hidden');
                permissionGranted.className = 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800';
            ...
            case 'denied':
                ...
                this.requestPermissionBtn.className = 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors';
                icon.className = 'fas fa-times mr-1';
            ...
            default:
                this.requestPermissionBtn.classList.remove('hidden');
                permissionGranted.classList.add('hidden');
                this.requestPermissionBtn.className = 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 hover:bg-yellow-200 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2 transition-colors';
...
if (document.readyState === 'loading') {
    ...
    console.log('NotificationSettings: DOM already loaded, initializing immediately...');
    window.notificationSettings = new NotificationSettings();
}
